...
class colorArray(list):
    def __init__(self, num_colors, kwargs):
        ...
        self.length = super().__len__() // self.n
    ...
    def __getitem__(self, a):
        ...
        if isinstance(a, slice):
                start = a.start*n if a.start != None else None
                stop = a.stop*n if a.stop != None else None
                return colorArray(n,
    ...
    def __setitem__(self, a, v):
        if isinstance(a, int):
            for i in range(self.n):
                super().__setitem__(a*self.n + i, v[i])
    def __len__(self):
        return self.length
    def reverse(self):
...
class ledEffect:
    def __init__(self, config):
        ...
        # When the effect is disabled we need to send one more update to disable the LEDs.
        self.lastUpdateApplied = False
    ...
    def _generateLayers(self, context=None):
        ...
        for layer in [line for line \
            ...
            self.layers.insert(0, layer(handler       = self,
                                        ...
                                        blendingMode  = self.blendingModes[parms[3]]))
    ...
    class layerChase(_layerBase):
        def __init__(self,  **kwargs):
            ...
            if self.effectRate == 0:
                self.thisFrame.append(list(chase[0:self.ledCount]))
            else:                                                   
            ...
            if int(self.effectRate/self.frameRate) == 0:
                ...
                for _ in range(len(self.paletteColors) * (self.ledCount-1)):
                    for _ in range(int(self.effectRate/self.frameRate)):
                        self.thisFrame.append(list(colorArray(COLORS, frame)[:COLORS*self.ledCount]))
    ...
    class layerHeater(_layerBase):
        ...
        def nextFrame(self, eventtime):
            frameHandler = self.frameHandler
            heater = self.handler.heater
            effectRate = self.effectRate

            heaterTarget  = frameHandler.heaterTarget[heater] # TODO
            heaterCurrent = frameHandler.heaterCurrent[heater] # TODO
            heaterLast    = frameHandler.heaterLast[heater]
    ...
    class layerTemperatureGauge(_layerBase):
        def __init__(self,  **kwargs):
            ...
            self.thisFrame.append(list(colorArray(COLORS, [0.0]*COLORS * self.ledCount)))
            self.steps = 255
            ...
            self.frameCount = len(list(self.thisFrame))
    ...
    class layerStepper(_layerBase):
        def __init__(self,  **kwargs):
            ...
            if self.effectRate == 0:
                trailing = np.zeros((1, COLORS))
            else:
                trailing = self._gradient(self.paletteColors[1:], int(self.effectRate), True)

            if self.effectCutoff == 0:
                leading = np.zeros((self.ledCount - len(trailing) - 1, COLORS))
            else:
                leading = np.concatenate((
                    self._gradient(self.paletteColors[1:], int(self.effectCutoff), False),
                    np.zeros((self.ledCount - len(trailing) - 1, COLORS)) # Long than chain so we can roll the array without bleed colours
                    ))

            gradient = np.concatenate((trailing, [self.paletteColors[0]], leading))
            gradient = np.roll(gradient, -len(trailing), axis=0)
            frames = [gradient[:self.ledCount]]

            for i in range(0, self.ledCount):
                gradient = np.roll(gradient, 1, axis=0)
                frames.append(gradient[:self.ledCount])

            for i in range(self.ledCount):
                self.frames = np.append(self.frames, [frames[i]], axis=0)

            self.frameCount = len(self.frames)
            self.led_step = self.ledCount / 101
        ...
        def nextFrame(self, eventtime):
            ...
            p = self.led_step * self.frameHandler.stepperPositions[axis]

            return self.frames[int(p)]
    ...
    class layerStepperColor(_layerBase):
        ...
        def nextFrame(self, eventtime):
            ...
            p = min(100,max(0,self.frameHandler.stepperPositions[int(axis)]*self.effectRate+self.effectCutoff))
    ...
    class layerProgress(_layerBase):
        def __init__(self,  **kwargs):
            ...
            self.thisFrame.append([0.0]*COLORS * self.ledCount)
            for i in range(1, 101):
                x = int((i / 101.0) * self.ledCount)
